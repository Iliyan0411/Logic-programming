...
graph([[a, b, c, d], [[a, b], [c, a], [b, c], [c, d]]]). % Example for graph
...
criticalVertex([V, E], X):- remove(X, V, newV), removeEdges(X, E, newE), not(isConnected([newV, newE])).

removeEdges(_, [], []).
removeEdges(X, [H|T], [H|R]):- not(member(X, H)), removeEdges(X, T, R).
removeEdges(X, [H|T], R):- member(X, H), removeEdges(X, T, R).
...
count(_, [], 0).
count(X, [X|T], N):- N1 is N-1, count(X, T, N1).
count(X, [H|T], N):- X \= H, count(X, T, N).

maxCount(L, X):- count(X, L, N), not(( member(Y, L), count(Y, L, M), N < M  )).
minCount(L, X):- count(X, L, N), not(( member(Y, L), count(Y, L, M), N > M  )).

diameter(L, D):- maxCount(L, X), minCount(L, Y), count(X, L, N), count(Y, L, M), D is N-M.

p(L):- not(( member(X, L), member(Y, L), diameter(X, D1), diameter(Y, D2), D1 =\= D2  )).
